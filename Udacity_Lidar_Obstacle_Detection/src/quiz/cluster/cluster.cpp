...
std::vector<int> clusterhelper(int i,const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol){
	processed[i] = true;
	cluster.push_back(i);
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	// Render clusters
  	int clusterId = 0;
	std::vector<Color> colors = {Color(0,0,0), Color(0,1,0), Color(0,0,1)};
